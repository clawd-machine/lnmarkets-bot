...
export class LNMClient {
  ...
  async getPositions() {
    ...
    return positions.map(pos => ({
      ...
      runningTime: pos.created_at || new Date(pos.creation_ts).toISOString(),
      sumCarryFees: pos.sum_carry_fees || 0, // Extract sum_carry_fees
    }))
  }
  ...
  async getOpenOrders() {
    ...
    // LNMarkets API: type='open' fetches open limit orders
  ...
  async cancelOrder(orderId) {
    if (!this.client) await this.initialize()
    return await this.client.futuresCancelTrade(orderId)
  }
  ...
  async newOrder(params) {
    ...
    // Only add price for limit orders ('l')
    // For market orders ('m'), price MUST be undefined
    if (orderParams.type === 'l' && params.price) {
    ...
    return {
      ...
      // For market orders, price might be in 'price' or 'entry_price' depending on API
      // But standardizing on 'entryPrice' for the bot
      entryPrice: parseFloat(result.price || result.entry_price || 0),
      price: parseFloat(result.price || result.entry_price || 0), // Keep for backward compatibility if needed
      leverage: result.leverage,
      margin: result.margin || 0,
      openingFee: result.opening_fee || 0
    }
  }
  ...
  async closePosition(positionId) {
    ...
    const result = await this.client.futuresCloseTrade(positionId)
