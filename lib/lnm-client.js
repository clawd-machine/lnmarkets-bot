...
export class LNMClient {
  ...
  isRetryableError(error) {
    // Network errors typically lack a response object
    if (!error.response) return true;
    
    const status = error.response.status;
    // Retry on 5xx server errors, 429 rate limits, 408 timeouts
  ...
  async logError(context) {
    const { operation, attempt, maxRetries, correlationId, error, params, isRetryable } = context;
    
    let category = 'internal';
    if (error.name === 'ValidationError') category = 'validation';
    else if (!error.response) category = 'network';
    else category = 'api';
    ...
    console.error(`[ERROR] [${correlationId}] ${category.toUpperCase()} Error in ${operation}: ${error.message}`);
    ...
    try {
      ...
      console.error('Failed to write to error.log', fsError.message);
    }
  }
   ...
   */
  async requestWithRetry(operationName, apiCall, params = null, maxRetries = 3) {
    let attempt = 0;
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        if (this.config.debug || true) {
          console.log(`[${correlationId}] Retrying ${operationName} in ${backoffMs}ms... (Attempt ${attempt}/${maxRetries})`);
        }
  ...
  validateOrder(params) {
    if (!params.type || !['limit', 'market', 'l', 'm'].includes(params.type)) {
      const err = new Error('Invalid order type. Must be limit, market, l, or m.');
      err.name = 'ValidationError';
      throw err;
    }
    if (!params.side || !['buy', 'sell', 'b', 's'].includes(params.side)) {
      const err = new Error('Invalid order side. Must be buy, sell, b, or s.');
      err.name = 'ValidationError';
      throw err;
    }
    ...
    if (params.leverage !== undefined && (params.leverage < 1 || params.leverage > 100)) {
      const err = new Error('Leverage must be between 1 and 100.');
      err.name = 'ValidationError';
      throw err;
    }
    ...
    if (orderType === 'limit') {
      if (!params.price || params.price <= 0) {
        const err = new Error('Price must be greater than 0 for limit orders.');
        err.name = 'ValidationError';
        throw err;
      }
    }
  ...
  async getBalance() {
    ...
    const user = await this.requestWithRetry('account.get', () => this.client.account.get())    
  ...
  async getCurrentPrice() {
    ...
    let ticker
    try {
      ticker = await this.requestWithRetry('futures.getTicket', () => this.client.futures.getTicket())
  ...
  async getPositions() {
    ...
    const positions = await this.requestWithRetry("futures.isolated.getRunningTrades", () => this.client.futures.isolated.getRunningTrades())
  ...
  async getOpenOrders() {
    ...
    const orders = await this.requestWithRetry('futures.isolated.getOpenTrades', () => this.client.futures.isolated.getOpenTrades())
  ...
  async cancelOrder(orderId) {
    if (!this.client) await this.initialize()
    return await this.requestWithRetry('futures.isolated.cancel', () => this.client.futures.isolated.cancel({ id: orderId }), { id: orderId })
  }
  ...
  async newOrder(params) {
    ...
    const result = await this.requestWithRetry('futures.isolated.newTrade', () => this.client.futures.isolated.newTrade(orderParams), orderParams)
  ...
  async openPosition({ side, quantity, leverage = 1, takeProfit = null, stopLoss = null }) {
    ...
    this.validateOrder(params);
    const result = await this.requestWithRetry('futures.isolated.newTrade', () => this.client.futures.isolated.newTrade(params), params)
  ...
  async closePosition(positionId) {
    ...
    const result = await this.requestWithRetry('futures.isolated.close', () => this.client.futures.isolated.close({ id: positionId }), { id: positionId })
  ...
  async getHistory({ limit = 100, from = null, to = null } = {}) {
    ...
    const response = await this.requestWithRetry('futures.isolated.getClosedTrades', () => this.client.futures.isolated.getClosedTrades(params), params)
  ...
  async healthCheck() {
    try {
      ...
      const [user, positions] = await Promise.all([
        this.requestWithRetry('account.get', () => this.client.account.get()),
        this.requestWithRetry("futures.isolated.getRunningTrades", () => this.client.futures.isolated.getRunningTrades())
      ])
