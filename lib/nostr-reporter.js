...
export class NostrReporter {
  ...
  async postTradeAlert(tradeInfo) {
    ...
    const message = `🪝 LNM Bot Rebalance ${emoji}\n` +
      `${actionText} @ ${(!isNaN(currentPrice) && currentPrice > 0) ? '$' + currentPrice.toLocaleString() : 'Market Price'}\n` +
      `Reason: ${reasoning}`;
  ...
  async postCloseAlert(closeInfo) {
    ...
    const message = `🪝 LNM Bot Position Closed ${emoji}\n` +
      `${actionText} @ ${(!isNaN(exitPrice) && exitPrice > 0) ? '$' + exitPrice.toLocaleString() : 'Market Price'}\n` +
